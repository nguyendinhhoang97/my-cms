/**
 * PUT /api/comments/:id/approve — duyệt bình luận (admin/editor).
 */
...
export const PUT: APIRoute = async (context) => {
	...
	if (!canManageTaxonomy(user)) {
		return errorResponse('Không có quyền duyệt bình luận.', 403);
	}
	...
	const updated = await updateCommentStatus(db, id, 'approved');
	if (!updated) {
		return errorResponse('Không thể duyệt bình luận.', 500);
	}
